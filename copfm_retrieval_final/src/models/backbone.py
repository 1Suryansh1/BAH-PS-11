...
copfm_repo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../copernicus_fm_src'))
...
class CopFMBackbone(nn.Module):
    def __init__(self, 
        ...
        def hook(module, input, output):
            # Store tokens keyed by the device ID of the output tensor
            device_id = output.device.index if output.device.type == 'cuda' else 'cpu'
            self._tokens_dict[device_id] = output
    ...
    def forward(self, 
        ...
        if mask is None:
            ...
            device_id = image.device.index if image.device.type == 'cuda' else 'cpu'
            tokens = self._tokens_dict.get(device_id)
            if tokens is None:
                raise RuntimeError(f"Hook failed to capture tokens on device {device_id}!")
            # Clear device entry to prevent memory leaks
            if device_id in self._tokens_dict:
                del self._tokens_dict[device_id]
