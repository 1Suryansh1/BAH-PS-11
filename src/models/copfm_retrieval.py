...
class CopFMRetrieval(nn.Module):
    ...
    def forward_train(self, batch_a, batch_b, wl_a, wl_b, bw_a, bw_b, mask_a, mask_b, meta_a=None, meta_b=None):
        ...
        # Step 2: encode full target images using shared online backbone
        with torch.no_grad():
            full_target_a = self.backbone(batch_a, wl_a, bw_a, return_patch_tokens=True, meta_info=meta_a)
            full_target_b = self.backbone(batch_b, wl_b, bw_b, return_patch_tokens=True, meta_info=meta_b)
            
            tgt_a = get_masked_tokens(full_target_a, mask_a)    # (B, N_mask, D)  — TARGET
            tgt_b = get_masked_tokens(full_target_b, mask_b)
        ...
        # Step 4: retrieval embeddings from VISIBLE token sequences (Eq 9 CR-JEPA)
        e_ca, r_ca = self.retrieval_heads.phi_cross(vis_a)
        e_cb, r_cb = self.retrieval_heads.phi_cross(vis_b)
        e_ua, r_ua = self.retrieval_heads.phi_uni(vis_a)
        e_ub, r_ub = self.retrieval_heads.phi_uni(vis_b)
    ...
    def forward(self, batch_a, batch_b, wl_a, wl_b, bw_a, bw_b, mask_a, mask_b, meta_a=None, meta_b=None):
        """
        Defers to forward_train during training. This allows torch.nn.DataParallel
        to correctly distribute batches and inputs across multiple GPUs.
        """
        return self.forward_train(batch_a, batch_b, wl_a, wl_b, bw_a, bw_b, mask_a, mask_b, meta_a=meta_a, meta_b=meta_b)
