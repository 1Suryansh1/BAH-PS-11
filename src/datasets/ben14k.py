...
class BEN14KDataset(Dataset):
    def __init__(self, root_dir, split='train', img_size=224, mask_ratio=0.50):
        """
        Args:
    ...
    def normalize_s2(self, s2_raw):
        ...
        # Handle 10-band datasets (drops B1 at idx 0 and B9 at idx 9)
        if s2.shape[0] == 10:
            indices_10 = [1, 2, 3, 4, 5, 6, 7, 8, 10, 11]
            mean = self.s2_mean[indices_10]
            std = self.s2_std[indices_10]
        else:
            mean = self.s2_mean
            std = self.s2_std
